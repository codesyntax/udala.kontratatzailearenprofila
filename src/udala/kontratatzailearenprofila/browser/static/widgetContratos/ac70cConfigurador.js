// Configurador Widget Contratos v1.0 [2021-01-13]
C = {
  ...
  iniciar: function () {
    ...
    for (var k in C.variables) {
      if (C.variables_defecto[k]) {
        C.variables_defecto[k] = C.variables[k];
      }
    }
  },
  ...
  genera_codigo_variables: function (obj_variables) {
    ...
    C.txt_codigo_variables = C.txt_codigo_variables.substr(
      0,
      C.txt_codigo_variables.lastIndexOf(",")
    );
    C.txt_codigo_variables += "\n}\n";
  },
  genera_codigo_final: function () {
    C.txt_codigo_final =
      '<script src="++plone++udala.kontratatzailearenprofila/widgetContratos/ac70cContratos.js" type="text/javascript">\n';
  ...
  carga_widget: function () {
    if (document.getElementById("jswidget").childNodes[0] != undefined) {
      document
        .getElementById("jswidget")
        .removeChild(document.getElementById("jswidget").childNodes[0]);
    }

    var j = document.createElement("script");
    j.type = "text/javascript";
    j.src =
      "++plone++udala.kontratatzailearenprofila/widgetContratos/ac70cContratos.js";
  ...
  coger_variables_URL: function () {
    ...
    if (ar_variables.length > 0) {
      ...
      for (i = 0; i < ar_variables.length; i++) {
        ...
        if (
          C.variables_defecto[Separ[0]] &&
          C.variables_defecto[Separ[0]] != Separ[1]
        ) {
  ...
  cargar_variables_defecto: function () {
    C.variables_defecto = {
      ...
      contratosMenores: document.getElementById("contratosMenores").value,
    };
  },
